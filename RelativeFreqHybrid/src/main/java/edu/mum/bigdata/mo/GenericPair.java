 ...
 */
public class GenericPair<S, T> implements WritableComparable<GenericPair> {
    S key;
    T val;
	...
	@Override
	public int compareTo(GenericPair o) {
        ...
        } else if(this.getKey() instanceof WritableComparable) {
        	int n = ((WritableComparable) this.key).compareTo((WritableComparable) gp.key);
            if(n != 0){
                return n;
            }
            return ((WritableComparable) this.val).compareTo((WritableComparable) gp.val);
